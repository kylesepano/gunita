  ...
  try {
    ...
    expect(
      (await db.query<{ count: number }>('select count(*)::int as count from event_sequence'))
        .rows[0].count,
    ).toBe(events.length * 4)
    await db.exec(
      `insert into auth.users values('00000000-0000-0000-0000-000000000001'),('00000000-0000-0000-0000-000000000002'); set role anon;`,
    )
    expect((await db.query('select * from historical_events')).rows.length).toBe(events.length)
    expect((await db.query('select * from historical_people')).rows.length).toBe(events.length)
    ...
    expect((await db.query('select * from historical_people')).rows).toHaveLength(events.length - 1)
  } finally {
    await db.close()
  }
}, 30000)
